...
**ターン完了は hook で受け取るのが確実です。** 画面検知は出力の静止で判定するため、エージェントが考えるために出力を止めただけでも `idle` に落ちて通知が出ます。エージェントはいずれもターンの終わりを自分で知っているので、[ターン完了を hook で通知する](#ターン完了を-hook-で通知する)の手順を入れてください。画面検知によるプロンプト復帰通知（`state.notify_on_idle`）は**既定でオフ**です。Claude Code がバックグラウンドタスクを実行している間は状態が `working` のままなので、この通知はバックグラウンド完了まで遅延します。stop hook 由来のターン完了通知は画面状態に依存しないため、ターン終了時に従来どおり発火します。
...
`webtabinal hooks print claude` の出力を `~/.claude/settings.json` にマージします。`Stop` はメインエージェントの応答完了時に発火します。バックグラウンドシェルやサブエージェントが残っている間、画面下端の `✻ … still running` 行を `working` として扱い、状態 pill は待機中に落ちません。画面検知のプロンプト復帰通知はバックグラウンド完了まで遅延しますが、この `Stop` hook はターン終了時に従来どおり発火します。
...
| 添付画像 | `~/Library/Application Support/WebTabinal/images/`（PNG / JPEG / GIF / WebP、1 枚 10 MiB まで、7 日で自動削除） |

## プロンプトへの画像添付

Claude Code / Codex / cursor-agent はいずれも画像を**ファイルパス**として受け取ります。そこで、ターミナルにペースト・ドロップされた画像はデーモンがファイルへ書き出し、ネイティブターミナルがドロップされたファイルのパスを打ち込むのと同じように、そのパスをプロンプトへ流し込みます。

| 操作 | 動作 |
|------|------|
| ターミナルへドラッグ&ドロップ | 画像ファイルを保存してパスを入力。ドロップ先が点線で示されます |
| Cmd+V | クリップボードに画像があればそれを添付。テキストなら従来どおりペースト |
| Ctrl+V | 素通し。Codex / Claude Code が自前で macOS のクリップボードを読む経路です |

対応形式は PNG / JPEG / GIF / WebP、1 枚あたり 10 MiB までです。判定はクライアントが名乗る Content-Type ではなく**バイト列**で行うため、拡張子を偽った投稿は 415 になります。SVG はスクリプトを含められるため受け付けません。

保存先は `~/Library/Application Support/WebTabinal/images/` で、7 日を過ぎたものはペースト時とデーモン起動時に削除します。削除対象はこの機能が生成した `img-<日時>-<8桁>.<拡張子>` という名前のファイルだけなので、同じディレクトリに置いた他のファイルは消えません。

パスにはバックスラッシュのエスケープが入ります（`Application\ Support`）。ネイティブターミナルがファイルをドロップしたときと同じ形式です。

複数枚を同時にドロップすると、スペース区切りでまとめて入力します。アップロードに失敗した画像は無言で飛ばし、成功した分のパスだけを入力します（失敗はブラウザのコンソールに出ます）。
...
xterm.js の `@xterm/addon-image` により、kitty graphics protocol / Sixel / iTerm2 inline image (IIP) のインライン画像を描画します。terminal-browser / terminal-code の起動時プローブ（`a=q,t=d` → `Gi=…;OK`）に応答します。共有メモリ・ファイル転送（`t=s` / `t=f`）はブラウザから読めないためエラー応答し、送信側はインライン転送へフォールバックします。

既知の制約:

- 再接続リプレイはリングバッファがバイト境界を無視するため、APC が途中から流れることがある
- `?1016`（ピクセル単位マウス）/ `?2048` / kitty keyboard protocol / `?2026` は未対応。マウス座標はセル単位になる
- WebTabinal は terminal-browser の既知端末リストに無い。Retina では CSI 14t が CSS ピクセルを返すため、画像スケールがずれることがある
- PTY → WebSocket（JSON + base64）経路はフレームレートが限られる。リアルタイム描画アプリの常用は想定していない
